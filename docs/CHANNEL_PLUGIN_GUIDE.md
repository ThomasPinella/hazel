...
Build a custom Hazel channel in three steps: subclass, package, install.

## How It Works

Hazel discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `hazel gateway` starts, it scans:

1. Built-in channels in `hazel/channels/`
2. External packages registered under the `hazel.channels` entry point group
...
```
hazel-channel-webhook/
├── hazel_channel_webhook/
...
```python
# hazel_channel_webhook/__init__.py
from hazel_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# hazel_channel_webhook/channel.py
import asyncio
...
class WebhookChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        await runner.setup()
        site = web.TCPSite(runner, "127.0.0.1", port)  # use 0.0.0.0 only behind a reverse proxy
        await site.start()
...
# pyproject.toml
[project]
name = "hazel-channel-webhook"
version = "0.1.0"
dependencies = ["hazel", "aiohttp"]

[project.entry-points."hazel.channels"]
webhook = "hazel_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
hazel plugins list      # verify "Webhook" shows as "plugin"
hazel onboard           # auto-adds default config for detected plugins
```

Edit `~/.hazel/config.json`:
...
| `default_config()` (classmethod) | Returns default config dict for `hazel onboard`. Override to declare your fields. |
...
Override `default_config()` so `hazel onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `hazel-channel-{name}` | `hazel-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `hazel_channel_{name}` | `hazel_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/hazel-channel-webhook
cd hazel-channel-webhook
pip install -e .
hazel plugins list    # should show "Webhook" as "plugin"
hazel gateway         # test end-to-end
```
