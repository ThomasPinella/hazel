...
import hazel.channels.dingtalk as dingtalk_module
from hazel.channels.dingtalk import DingTalkChannel, HazelDingTalkHandler
from hazel.channels.dingtalk import DingTalkConfig
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = HazelDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/hazel_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/hazel_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "hazel.config.paths.get_media_dir",
