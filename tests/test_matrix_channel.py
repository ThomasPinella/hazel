...
import hazel.channels.matrix as matrix_module
from hazel.bus.events import OutboundMessage
from hazel.bus.queue import MessageBus
from hazel.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from hazel.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("hazel.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "hazel.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("hazel.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "hazel.channels.matrix.asyncio.create_task", _fake_create_task
    )
